...
def GetPhraseSentences(sentences, phrase_list):
  match_sentences = [ ]
  line_numbers = [ ]

  for sentence in sentences:
    for phrase in phrase_list:
      if sentence.val.find(phrase) >= 0:
        line_numbers.append(sentence.line_number)
...
if len(sys.argv) < 8:
  ...
  print "    word_list: a file with the words/phrases to search for"
...
phrases = [ ]
with open(word_list) as phrase_file:
  for phrase in phrase_file:
    phrases.append(phrase[0:len(phrase) - 1])
...
with open(language_list) as language_file:
  for language in language_file:
    ...
    (examples, line_numbers) = GetPhraseSentences(sentences, phrases)  

    if len(examples) > 0:
      print "Found %d examples for language %s" % (len(line_numbers), long_name)
      output_prefix = long_name + "phrases" 
