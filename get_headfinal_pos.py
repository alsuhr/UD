...
def GetMatches(sentence, dep_type):
  line_numbers = [ ]

  for word in sentence.words:
    if word.pos == dep_type and word.p_dep < word.id:
        line_numbers.append(sentence.line_number + word.id)

  return line_numbers

def GetHeadfinalSentences(sentences, dep_type):
  match_sentences = [ ]
  line_numbers = [ ]

  for sentence in sentences:
    if sentence.ContainsPOS(dep_type):
      ex_ln = GetMatches(sentence, dep_type)
      if len(ex_ln) > 0:
...
if len(sys.argv) < 6:
  ...
  print "      e: just example sentences; one file will be created for each language in the form language_dep_type_headfinal_ex.txt"
  print "      l: just line numbers; one file will be created for each language in the form language_dep_type_headfinal_ln.txt"
...
with open(language_list) as language_file:
  for language in language_file:
    ...
    (examples, line_numbers) = GetHeadfinalSentences(sentences, dep_type) 

    if len(examples) > 0:
      print "Found %d examples for language %s with pos %s" % (len(line_numbers), long_name, dep_type)
      output_prefix = long_name + "_" + dep_type + "_headfinal" 
      ...
      print "No such examples for language %s with deptype %s!" % (long_name, dep_type)
